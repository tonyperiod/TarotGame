...
public class WheelOfFortune : MonoBehaviour
{
    ...
    public void Activate(GameObject c, bool isPlayer)
    {
        ...
        //add card to player scriptable database, with enemy add to current list of cards
        if (isPlayer == true)
        {
            int randomSlot = Random.Range(0, shopAllDataInGame.allCards.Count);
            pDeck.playerDatabase.allCards.Add(shopAllDataInGame.allCards[randomSlot]);//add to player database PERMANENT IN SESSION
            shopAllDataInGame.allCards.Remove(shopAllDataInGame.allCards[randomSlot]);//remove to shopalldata FOR THE SESSION
            slot = 9;
        }
